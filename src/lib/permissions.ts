...
export async function requirePermission(req: Request, permission: keyof typeof permissions) {
    ...
    if (!payload || !hasPermission(payload.role as string, permission)) {
        throw new Error("Unauthorized");
    }

    return { user: { id: payload.id as string, email: payload.email as string, role: payload.role as string } };
}
